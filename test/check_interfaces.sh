...
for name in domain transaction_as_connection; do
  ...
  cat "$name.out"
  case "$name" in
    domain) grep -q 'connection' "$name.out"; grep -q '"contended"' "$name.out"; grep -q '"uncontended"' "$name.out"; grep -q '"portable"' "$name.out" ;;
    transaction_as_connection) grep -q 'Duckdb.transaction' "$name.out"; grep -q 'Duckdb.connection' "$name.out" ;;
  esac
done
