...
for name in return store capture domain inner_effect owner_close owner_reset; do
  ...
  cat "$name.out"
  case "$name" in
    owner_close) grep -q 'Duckdb.chunk' "$name.out"; grep -q 'Duckdb.query_result' "$name.out" ;;
    owner_reset) grep -q 'Duckdb.chunk' "$name.out"; grep -q 'Duckdb.prepared' "$name.out" ;;
    *) grep -q '"local"' "$name.out"; grep -q '"global"' "$name.out" ;;
  esac
done
