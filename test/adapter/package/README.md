...
```sh
bash test/adapter/package/check.sh interface-red # expected exit 1: missing implementations
bash test/adapter/package/check.sh latch-red     # expected exit 2: generated latch mutation
bash test/adapter/package/check.sh green         # expected exit 0
```

Each invocation creates a fresh source/build/prefix tree under ignored
`.local/package-fixtures/`; `logs/*-work.txt` identifies it. The runner sets
...
Next execution plan: `docs/architecture.md`.
