...
base="$root/.local/stage4b/packaging"
mkdir -p "$base/logs"
...
for project in core consumer; do
  for source in "stage4/packaging/$project/"*.in; do
...
for source in stage4/packaging/negative/*.ml.fail; do
...
for file in META dune-package; do
  if grep -F -e "$work/core" -e "$root/stage4" -e "$root/_build" "$prefix/lib/packaging-core/$file"; then
