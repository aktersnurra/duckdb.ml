...
static void clear_result(prepared_owner *p) {
    if (!p) return;
...
static void clear_prepared(prepared_owner *p) {
    if (!p) return;
    clear_result(p);
...
static void delete_prepared(prepared_owner *p) {
    if (!p) return;
    clear_prepared(p); duckdb_ml_connection_unref(p->parent); free(p); duckdb_ml_released();
}
...
CAMLprim value ml_duckdb_execute_prepared(value v) {
    ...
    caml_enter_blocking_section();
    p->status = 0; p->has_result = 1; duckdb_ml_acquired();
    if (duckdb_execute_prepared(p->prepared, &p->result) != DuckDBSuccess)
        set_error(p, duckdb_result_error(&p->result));
    caml_leave_blocking_section(); caml_process_pending_actions(); CAMLreturn(Val_unit);
}
...
CAMLprim value ml_duckdb_close_result(value v) {
    CAMLparam1(v); prepared_owner *p = Prepared(v);
    caml_enter_blocking_section(); clear_result(p); caml_leave_blocking_section();
    caml_process_pending_actions(); CAMLreturn(Val_unit);
}
CAMLprim value ml_duckdb_finish_result_close(value v) { clear_result(Prepared(v)); return Val_unit; }
CAMLprim value ml_duckdb_close_prepared(value v) {
    CAMLparam1(v); prepared_owner *p = Prepared(v);
    caml_enter_blocking_section(); clear_prepared(p); caml_leave_blocking_section();
    caml_process_pending_actions(); CAMLreturn(Val_unit);
}
