module Bindings (F : Ctypes.FOREIGN) : sig
  val query_into :
    (char Ctypes.ptr -> unit Ctypes.ptr Ctypes.ptr -> unit F.return) F.result
...
(** Short native-only cleanup, generated without a runtime-lock transition. *)
module Cleanup (F : Ctypes.FOREIGN) : sig
  val destroy_slot : (unit Ctypes.ptr Ctypes.ptr -> unit F.return) F.result
end
